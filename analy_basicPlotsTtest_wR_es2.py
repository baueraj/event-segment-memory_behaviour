...
fileSavePath = '../results/meetKamy_101017/'

allDat = get_participant_data(aPs, cPs, paths)
...
for i in range(len(dat_plot)):
    ...
    if i % 2 == 0:
        pltTitle = cartoonNames[int(np.floor(i/2))] + '_acc'
        ax.set_xticklabels(plt_acc_Xlabels, fontsize=20)
        ax.set_ylim([0.4, 1.0])
    else:
        pltTitle = cartoonNames[int(np.floor(i/2))] + '_RT'
        ax.set_xticklabels(plt_RT_Xlabels, fontsize=20)
        ax.set_ylim([900, 1600])

    plt.title(pltTitle, fontsize=20)
    plt.yticks(fontsize=18)    
    plt.savefig(fileSavePath + 'barplots/fig_{0}_{1}.jpg'.format(i,pltTitle))
...
for i in range(len(dat_bPlot)):
    ...
    plt.title(pltTitle, fontsize=20)
    plt.yticks(fontsize=18)       
    plt.savefig(fileSavePath + 'boxplots/fig_{0}_{1}.jpg'.format(i,pltTitle))
